...
These numbers are a sanity check for the current `research_debug` checkpoint, not a strength claim.
They were measured on 2026-05-19 on an Apple M3 Pro with PyTorch `2.10.0` using MPS. The checkpoint
was generated by `./scripts/smoke_macbook.sh` with `configs/research_debug.yaml`.

Self-play/training smoke:

| Metric | Value |
| --- | ---: |
| self-play games | 1 |
| self-play positions | 8 |
| self-play average plies | 8.0 |
| self-play average nodes/s | 42.96 |
| policy loss | 9.13 |
| value loss | 0.073 |
| legal move accuracy | 0.125 |
| target move accuracy | 0.0 |

Evaluation from the same checkpoint:

| Opponent | Games | Simulations | Max plies | Result | Score mean | Avg plies |
| --- | ---: | ---: | ---: | --- | ---: | ---: |
| random | 4 | 4 | 40 | 2W / 1D / 1L | 0.25 | 40.0 |
| material baseline | 4 | 4 | 40 | 0W / 0D / 4L | -1.0 | 40.0 |
| Pikafish 2026-01-02 depth 1 | 2 | 4 | 30 | 0W / 0D / 2L | -1.0 | 14.5 |

Interpretation: the pipeline runs, but the debug checkpoint is weak and already loses to a simple
material baseline and low-depth Pikafish. The next real strength step is supervised data or
Pikafish distillation, then self-play.
